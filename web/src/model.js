...
export function entity(type = 'stroke') {
  return { id: crypto.randomUUID(), type, brush: 'Pen', pattern:'solid', size:[.5,.5,.5], paperKind:'watercolor',paperId:'',position: [0,0,0], normal: [0,0,1], yaw: 0, scale: 1, width: .018, alpha: 1, arc: 100, panelWidth: 2, aspect: 2, color: 0xff61e8c6 | 0, wet: false, surface: false, image: '', points: [] };
}
export const versionOf = entities => entities.some(e=>e.type==='paper'||e.paperId)?3:entities.some(e=>e.type==='block'||(e.pattern&&e.pattern!=='solid'))?2:1;
...
export function validate(doc) {
  need(doc && [1,2,3].includes(doc.version) && doc.units === 'metres' && doc.coordinates === 'right-handed-y-up', 'This file is not a supported Draw in 3D project.');
  ...
  for (const e of doc.entities) {
    ...
    for (const [key,lo,hi] of [['yaw',-36000,36000],['scale',.1,4],['width',.002,.15],['alpha',.05,1],['arc',0,300],['panelWidth',.1,4],['aspect',.1,10]]) number(e[key],lo,hi,key);
    ...
    for (const p of e.points) { vector(p,4,8); number(p[3],.1,2,'pressure'); }
...
export function encode(entities) {
  ...
  const compatible = entities.map(e => {
    const copy = clone(e);
    for (const key of ['yaw','scale','width','alpha','arc','panelWidth','aspect']) copy[key] = Math.fround(e[key]);
