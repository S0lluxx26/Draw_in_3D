...
export function entity(type = 'stroke') {
  return { id: crypto.randomUUID(), type, brush: 'Pen', pattern:'solid', size:[.5,.5,.5], paperKind:'watercolor',paperName:'',paperVisible:true,paperId:'',pointSpace:'object',bend:0,pitch:0,roll:0,position: [0,0,0], normal: [0,0,1], yaw: 0, scale: 1, width: .018, alpha: 1, arc: 100, panelWidth: 2, aspect: 2, color: 0xff61e8c6 | 0, wet: false, surface: false, image: '', points: [] };
}
export const versionOf = entities => entities.some(e=>e.paperName)?6:entities.some(e=>e.type==='paper'&&e.paperVisible===false)?5:entities.some(e=>e.pointSpace==='surface'||e.bend||e.pitch||e.roll)?4:entities.some(e=>e.type==='paper'||e.paperId)?3:entities.some(e=>e.type==='block'||(e.pattern&&e.pattern!=='solid'))?2:1;
...
export function validate(doc) {
  ...
  for (const e of doc.entities) {
    ...
    need(e.paperName===undefined||typeof e.paperName==='string'&&e.paperName.length<=64&&!/[\u0000-\u001f\u007f]/.test(e.paperName),'Sheet names must be at most 64 characters, without control characters.');
    need(!e.paperName||e.type==='paper'&&doc.version>=6,'Named sheets need a paper entity and project version 6.');
...
export function encode(entities) {
  ...
  const compatible = entities.map(e => {
    ...
    if(!e.paperName)delete copy.paperName;
