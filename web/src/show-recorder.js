...
export class ShowRecorder{
  ...
  start(){
    ...
    try{
      ...
      this.saved={camera:p.camera.clone(),target:p.orbit.target.clone(),ratio:p.renderer.getPixelRatio(),rate:p.clock.rate};
      ...
      recorder.ondataavailable=e=>{if(e.data.size&&!this.cancelled){this.bytes+=e.data.size;if(this.bytes>160*1024*1024){this.cancelled=true;this.chunks=[];this.finish(true,'Video exceeded the 160 MiB recording budget. Try a shorter show.');return;}this.chunks.push(e.data);}};
      recorder.onerror=()=>{this.cancelled=true;this.chunks=[];this.finish(true,'Recording failed. Try a shorter show or another browser.');};
      recorder.onstop=()=>{
