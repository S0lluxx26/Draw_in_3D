...
export class ShowRecorder{
  ...
  reset(){const supported=this.available();$('show-record').disabled=!supported||!!this.player.waiting;$('show-record').textContent='Record video';$('show-record-status').textContent=supported?(this.player.music?.enabled?'Video with music':'Silent video')+' · real time · keep this tab visible':'Canvas recording is unavailable in this browser.';}
  start(){
    ...
    try{
      ...
      const recorder=this.recorder=new MediaRecorder(this.stream,{mimeType:mime,videoBitsPerSecond:Math.round(bits-(audio?AUDIO_BITS:0)),...audio?{audioBitsPerSecond:AUDIO_BITS}:{}});
      ...
      this.lock(true);recorder.start(1000);p.clock.play(performance.now());$('show-record').textContent='Cancel recording';$('show-record-status').textContent=`Recording from takeoff${audio?' with music':''}… keep this tab visible.`;p.refresh();
