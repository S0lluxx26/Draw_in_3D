...
import {attachedIds} from './paper.js';
...
export function pivotOf(entities,ids){const chosen=entities.filter(e=>ids.has(e.id));return chosen.length?[0,1,2].map(i=>chosen.reduce((n,e)=>n+e.position[i],0)/chosen.length):[0,0,0];}
export function transformSelection(entities,ids,{move=[0,0,0],yaw=0,scale=1,pivot=pivotOf(entities,ids)}={}){
  ids=attachedIds(entities,ids);
  const angle=yaw*Math.PI/180,c=Math.cos(angle),s=Math.sin(angle);
  const next=entities.map(e=>{
    if(!ids.has(e.id))return e;const copy=clone(e),p=e.position.map((v,i)=>(v-pivot[i])*scale);
    copy.position=[p[0]*c+p[2]*s,p[1],-p[0]*s+p[2]*c].map((v,i)=>v+pivot[i]+move[i]);copy.yaw+=yaw;copy.scale*=scale;return copy;
...
export function duplicateSelection(entities,ids){
  ...
  const copies=entities.filter(e=>ids.has(e.id)).map(e=>({...clone(e),id:mapping.get(e.id),paperId:mapping.get(e.paperId)||e.paperId,position:e.position.map((v,i)=>v+(i<2?.15:0))}));
...
export function eraseStroke(e,project,from,to,radius){
  ...
  for(let i=0;i<e.points.length-1;i++){
    const a=e.points[i],b=e.points[i+1],pa=project(e,a),pb=project(e,b),cuts=pa&&pb?cutIntervals(pa,pb,from,to,radius):[];
  ...
  return chunks.map((points,i)=>({...clone(e),id:i?crypto.randomUUID():e.id,points}));
}
