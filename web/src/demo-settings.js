...
export function installDemoSettings({play,player,notify}){
  ...
  dialog.innerHTML=`<form><h2 id="demo-settings-title">Demo settings</h2><p>Blender-built 3D formations. Settings apply to this Demo; saved shows stay independent.</p>
    ...
    <label>Formation size / spacing<select id="demo-scale"><option value="2">4Ã— original size</option><option value="3">6Ã— original size (default)</option><option value="4">8Ã— original size</option></select></label>
  ...
  dialog.querySelector('form').onsubmit=async e=>{e.preventDefault();const request=++generation,submit=e.submitter;submit.disabled=true;$('demo-settings-status').textContent='Preparing formationsâ€¦';
