...
try{
  ...
  await page.locator('#show-editor').click();await page.locator('#author-dialog').waitFor({state:'visible'});assert.equal(await page.locator('.author-card').count(),12);
  ...
  // Convert the Robot to a drawing on the stage, then open it in the 3D workspace.
  await page.locator('#author-edit').click();assert.equal(await page.locator('#author-edit').textContent(),'Edit in 3D workspace');await page.locator('#author-edit').click();await page.locator('#formation-edit-banner').waitFor({state:'visible'});
  ...
  await page.locator('#author-duplicate').click();assert.equal(await page.locator('.author-card').count(),13);await page.locator('#author-undo').click();assert.equal(await page.locator('.author-card').count(),12);await page.locator('#author-redo').click();await page.locator('#author-delete').click();assert.equal(await page.locator('.author-card').count(),12);
  ...
  // Edit demo opens the Demo's 3D formations; editing the Robot converted it to its line art first.
  const robot=edited.cues.find(c=>c.name==='Robot');assert.equal(demo.version,3);assert.equal(demo.cues[0].library,'Robot');assert.equal(robot.library,undefined);assert.ok(robot.artwork.filter(e=>e.type==='stroke').length>5);assert.equal(robot.artwork[1].color,0xffff0088|0);assert.deepEqual(robot.placement,{origin:[0,1.4,0],scale:20,position:[0,24,0],yaw:0},'on the designer stage, where the Demo formation was');assert.ok(edited.cues.filter(c=>c.library).length>=10,'the other formations stay Demo formations');
  ...
  // Draw a formation on the designer stage (no trip to the 3D workspace).
  await page.locator('#author-blank').click();await page.locator('[data-draw=ellipse]').click();box=await page.locator('#author-preview').boundingBox();await page.mouse.move(box.x+box.width*.4,box.y+box.height*.45);await page.mouse.down();await page.mouse.move(box.x+box.width*.6,box.y+box.height*.6,{steps:10});await page.mouse.up();assert.equal(await page.locator('.author-card').count(),13);await page.locator('#author-fit').click();await page.locator('#author-formation-preview').click();await page.locator('#drone-show').waitFor({state:'visible'});await page.waitForFunction(()=>document.querySelector('#show-phase').textContent==='Forming New formation');/* Run from here starts as it forms */await page.locator('#show-exit').click();
