...
try{
  ...
  for(const [label,phase]of [['Robot','Robot'],['Fish','Fish'],['Eiffel Tower','Eiffel Tower'],['Fireworks','Fireworks'],['Landing','Landing']]){
  ...
  await page.locator('#show-scrub').fill('87');await page.locator('#show-scrub').dispatchEvent('input');await page.waitForFunction(()=>document.querySelector('#show-play-state').textContent==='SHOW COMPLETE');assert.equal(await page.locator('#show-lit').textContent(),'0 / 256');
